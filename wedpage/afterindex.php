...
if (isset($_SESSION['id'])) {  // เงื่อนไข if ถ้า มี isset($_SESSION['id'] จะทำให้เงื่อนไขนี้เป็นจริง true
  $query = "SELECT * FROM users WHERE id = '$_SESSION[id]' ";  //การกำหนดค่า ตัวแปล loginAction ใหัมีค่า true
  ...
  if (mysqli_num_rows($result) == 1) {  // การแปลงข้อมูล result ที่ได้มาให้อยู่ในรูปแบบ array จะถูกเก็บไว้ในตัวแปล userdata
    $_SESSION['status'] = $userdata['status'];  //  เงื่อนไข if ถ้ามีการพบข้อมูลที่มี length เท่ากับ 1 จะทำให้เงื่อนไขนี้เป็นจริง true
  }  //การกำหนดค่า ตัวแปล $_SESSION['status'] ใหัมีค่า $userdata['status']
}
